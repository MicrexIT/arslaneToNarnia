defmodule ArslaneToNarnia.Logs do
  @moduledoc """
  ...
  def create(attrs \\ %{}) do
    case %Log{}
         ...
         |> Repo.insert() do
      {:ok, log} ->
        ArslaneToNarniaWeb.Endpoint.broadcast("logs", "new-log", log)
        {:ok, log}
  ...
  def log_async(action, params, throttle_seconds \\ 0) do
    ArslaneToNarnia.BackgroundTask.run(fn ->
  ...
  def exists?(params) do
    ...
    |> ArslaneToNarnia.Repo.exists?()
  end

  def get_last_log_of_user(user) do
    ArslaneToNarnia.Logs.LogQuery.by_user(user.id)
    |> ArslaneToNarnia.Logs.LogQuery.order_by(:newest)
    |> DB.limit(1)
    |> ArslaneToNarnia.Repo.one()
  end
end
