defmodule ArslaneToNarniaWeb.UserOnboardingLive do
  ...
  def handle_event("submit", %{"user" => user_params}, socket) do
    ...
    case Accounts.update_profile(socket.assigns.current_user, user_params) do
      {:ok, updated_user} ->
        ArslaneToNarnia.Logs.log_async("update_profile", %{
          user: updated_user,
          profile_user: updated_user
        })

        ArslaneToNarnia.MailBluster.sync_user_async(updated_user)

        ArslaneToNarnia.Slack.message("""
        ...
        *Subscribed to marketing emails*: #{updated_user.is_subscribed_to_marketing_notifications}
        ...
        #{ArslaneToNarniaWeb.Endpoint.url()}/admin/users/#{updated_user.id}
        """)
