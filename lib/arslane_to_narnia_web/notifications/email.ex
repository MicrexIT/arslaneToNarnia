defmodule ArslaneToNarnia.Email do
  @moduledoc """
  ...
  An Swoosh email struct can be delivered by a Swoosh mailer (see mailer.ex & user_notifier.ex). Eg:
      ...
      ArslaneToNarnia.Email.confirm_register_email(user.email, url)
      |> ArslaneToNarnia.Mailer.deliver()
  """

  use Phoenix.Swoosh,
    view: ArslaneToNarniaWeb.EmailView,
    layout: {ArslaneToNarniaWeb.EmailView, "email_layout.html"}
  ...
  defp from_name do
    Application.get_env(:arslane_to_narnia, :mailer_default_from_name)
  end

  defp from_email do
    Application.get_env(:arslane_to_narnia, :mailer_default_from_email)
  end
  ...
  defp get_unsubscribe_url(user, email_kind) do
    ArslaneToNarnia.Accounts.NotificationSubscriptions.unsubscribe_url(user, email_kind)
  end
end
