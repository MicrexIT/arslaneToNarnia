defmodule ArslaneToNarniaWeb.Menus do
  @moduledoc """
  ...
  alias ArslaneToNarniaWeb.Endpoint
  alias ArslaneToNarniaWeb.Helpers
  ...
  def get_link(:settings, _current_user) do
    %{
      ...
      path: Routes.live_path(Endpoint, ArslaneToNarniaWeb.EditProfileLive),
      icon: :cog
    }
  end
  ...
  def get_link(:edit_email, _current_user) do
    %{
      ...
      path: Routes.live_path(Endpoint, ArslaneToNarniaWeb.EditEmailLive),
      icon: :at_symbol
    }
  end
  ...
  def get_link(:edit_notifications, _current_user) do
    %{
      ...
      path: Routes.live_path(Endpoint, ArslaneToNarniaWeb.EditNotificationsLive),
      icon: :bell
    }
  end
  ...
  def get_link(:edit_password, _current_user) do
    %{
      ...
      path: Routes.live_path(Endpoint, ArslaneToNarniaWeb.EditPasswordLive),
      icon: :key
    }
  end
  ...
  def get_link(:dashboard, _current_user) do
    %{
      ...
      # path: Routes.live_path(Endpoint, ArslaneToNarniaWeb.ProjectPublicLive.Index),
      path: Routes.project_public_index_path(Endpoint, :index),
      icon: :template
    }
  end
  ...
  def get_link(:admin_projects = name, current_user) do
    if Helpers.is_admin?(current_user) do
      %{
        name: name,
        label: "Projects",
        path: Routes.project_index_path(Endpoint, :index),
        icon: :sun
      }
    else
      nil
  ...
  def get_link(:dev_email_templates = name, _current_user) do
    if Application.get_env(:arslane_to_narnia, :env) == :dev do
      %{
        name: name,
  ...
  def get_link(:dev_page_builder = name, _current_user) do
    if Application.get_env(:arslane_to_narnia, :env) == :dev do
      %{
        name: name,
        label: "Page builder",
        path: Routes.live_path(Endpoint, ArslaneToNarniaWeb.PageBuilderLive),
        icon: :document_add
      }
    else
      nil
