defmodule ArslaneToNarniaWeb.Components.Brand do
  ...
  def logo(assigns) do
    ...
    ~H"""
    <%= if @variant do %>
      <img class={@class} src={"/images/arslane_#{@variant}.svg"} />
    <% else %>
  ...
  def logo_for_emails(assigns) do
    ~H"""
    <img height="60" src={Application.get_env(:arslane_to_narnia, :logo_url_for_emails)} />
    """
  end
end
