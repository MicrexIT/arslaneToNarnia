defmodule ArslaneToNarniaWeb.UserAuth do
  @moduledoc """
  ...
  alias ArslaneToNarnia.{Repo, Accounts}
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_arslane_to_narnia_web_user_remember_me"
  ...
  def log_in_user(conn, user, params \\ %{}) do
    ...
    ArslaneToNarnia.Logs.log_async("sign_in", %{user: user})
    ArslaneToNarnia.MailBluster.sync_user_async(user)
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ArslaneToNarniaWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end

    if conn.assigns[:current_user] do
      ArslaneToNarnia.Logs.log_async("sign_out", %{user: conn.assigns.current_user})
    end
  ...
  def require_authenticated_user(conn, _opts) do
    if conn.assigns[:current_user] do
      ...
      |> redirect(to: ArslaneToNarniaWeb.Router.Helpers.user_session_path(conn, :new))
      |> halt()
    end
  end
  ...
  defp signed_in_path(current_user), do: ArslaneToNarniaWeb.Helpers.home_path(current_user)
