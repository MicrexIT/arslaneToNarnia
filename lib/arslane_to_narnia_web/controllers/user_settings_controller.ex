defmodule ArslaneToNarniaWeb.UserSettingsController do
  use ArslaneToNarniaWeb, :controller

  alias ArslaneToNarnia.Accounts
  alias ArslaneToNarniaWeb.UserAuth
  alias ArslaneToNarnia.Accounts.NotificationSubscriptions
  ...
  def update_password(conn, params) do
    ...
    case Accounts.update_user_password(user, password, user_params) do
      {:ok, user} ->
        ...
        |> put_session(:user_return_to, Routes.live_path(conn, ArslaneToNarniaWeb.EditPasswordLive))
        |> UserAuth.log_in_user(user)

      {:error, changeset} ->
        conn
        |> put_flash(:error, ArslaneToNarniaWeb.ErrorHelpers.combine_changeset_error_messages(changeset))
        |> redirect(to: Routes.live_path(conn, ArslaneToNarniaWeb.EditPasswordLive))
    end
  end
  ...
  def confirm_email(conn, %{"token" => token}) do
    case Accounts.update_user_email(conn.assigns.current_user, token) do
      :ok ->
        ...
        |> redirect(to: Routes.live_path(conn, ArslaneToNarniaWeb.EditProfileLive))
